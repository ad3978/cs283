...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        //  - etc.
        char rc_buff[128];
        sprintf(rc_buff, RCMD_MSG_SVR_RC_CMD, cmd_rc);
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (pids[i] < 0) {
            ...
            // Set up stdin
            if (i == 0) {
                // First process gets stdin from the socket
                dup2(cli_sock, STDIN_FILENO);
            } else {
                // Not the first process, get stdin from the previous pipe
                dup2(pipes[i-1][0], STDIN_FILENO);
            }
            
            // Set up stdout
            if (i == clist->num - 1) {
                // Last process sends stdout to the socket
                dup2(cli_sock, STDOUT_FILENO);
                dup2(cli_sock, STDERR_FILENO);
            } else {
                // Not the last process, send stdout to the next pipe
                dup2(pipes[i][1], STDOUT_FILENO);
                // For intermediate processes, stderr still goes to the socket
                dup2(cli_sock, STDERR_FILENO);
            }
